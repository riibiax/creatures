...
var Ribbon = function(bounds, emitter_count){
	...
	this.init = function(){
		...
		this.LEN = 750; //number of spine points

		this.velocity = new THREE.Vector3();
		this.speed = ATUtil.randomRange(.5, 2.);
		this.ribbonWidth = ATUtil.randomRange(.75, 2.);
		...
		this.noiseScale = 5000;// 100, 5000).name('Turbulence');
		this.noiseSpeed =  .0001;//', 0, 0.01).name('Variance');
		this.noiseSeparation = 1.5; //', 0, 0.5).name('Cohesion');
		this.ribbonSpeed = .05; //', 0.1, 5).name('Speed');
	...
	this.reset = function(){
		...
		if (Math.random() < this.clumpiness){
			...
			this.head.addVectors(emitters[emitterId],ATUtil.randomVector3(bounds));
		}else{
	...
	this.update = function(){
		...
		this.velocity.x = noise.noise4d(this.vec.x, this.vec.y, this.vec.z, noiseTime + this.id ) * this.speed * this.ribbonSpeed;
		this.velocity.y = noise.noise4d(this.vec.x, this.vec.y, this.vec.z, noiseTime + this.id * 5. ) * this.speed * this.ribbonSpeed;
		this.velocity.z = noise.noise4d(this.vec.x, this.vec.y, this.vec.z, noiseTime + this.id * 10.) * this.speed * this.ribbonSpeed;
